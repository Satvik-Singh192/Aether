...
namespace
{
	...
	static void orthonormalBasis(const glm::vec3 &dir, glm::vec3 &u, glm::vec3 &v)
	{
		glm::vec3 up = (std::abs(dir.y) < 0.9f) ? glm::vec3(0.0f, 1.0f, 0.0f) : glm::vec3(1.0f, 0.0f, 0.0f);
		u = glm::normalize(glm::cross(up, dir));
		v = glm::cross(dir, u);
	}
							 ...
							 int slices)
	{
		...
		if (len < 1.0e-5f || radius < 1.0e-6f)
			return;
		glm::vec3 dir = d * (1.0f / len);
		glm::vec3 u, w;
		orthonormalBasis(dir, u, w);
		glm::vec3 nBottom = -dir;
		glm::vec3 nTop = dir;
		for (int i = 0; i < slices; ++i)
		{
			float t0 = (float)i / (float)slices * 6.28318530718f;
			float t1 = (float)(i + 1) / (float)slices * 6.28318530718f;
			glm::vec3 c0 = std::cos(t0) * u + std::sin(t0) * w;
			glm::vec3 c1 = std::cos(t1) * u + std::sin(t1) * w;
			glm::vec3 ab0 = a + radius * c0;
			glm::vec3 ab1 = a + radius * c1;
			glm::vec3 bb0 = b + radius * c0;
			glm::vec3 bb1 = b + radius * c1;
			...
			pushTri(v, b, nTop, bb1, nTop, bb0, nTop);
		}
	}
	...
	static void pushSpringSolid(std::vector<float> &v, const glm::vec3 &a, const glm::vec3 &b, float tubeRadius,
							 float coilRadius, int numCoils, int segmentsAlong, int cylinderSlices)
	{
		glm::vec3 d = b - a;
		float len = glm::length(d);
		if (len < 1.0e-5f)
	...
	static void constraintBaseColor(ConstraintType type, float &r, float &g, float &b)
	{
		if (type == ConstraintType::Rope)
		{
			r = 0.92f;
			g = 0.78f;
			b = 0.28f;
		}
		else if (type == ConstraintType::Rod)
		{
			r = 0.32f;
			g = 0.82f;
			b = 0.98f;
	...
	static std::vector<float> buildSolidConstraint(const DistanceConstraintState &constraint, const glm::vec3 &p0, const glm::vec3 &p1)
	{
		std::vector<float> solid;
		solid.reserve(4096);
		if (constraint.type == ConstraintType::Rope)
			pushCylinderSolid(solid, p0, p1, 0.045f, 12);
		else if (constraint.type == ConstraintType::Rod)
	...
	static std::vector<float> buildWireConstraint(const DistanceConstraintState &constraint, const glm::vec3 &p0, const glm::vec3 &p1)
	{
    ...
    float tintB)
{
	...
	if (smFogNear >= 0)
		glUniform1f(smFogNear, 28.0f);
	if (smFogFar >= 0)
		glUniform1f(smFogFar, 80.0f);

	const glm::vec3 specularColor(0.95f, 0.97f, 1.0f);
	glBindVertexArray(vao);
	glBindBuffer(GL_ARRAY_BUFFER, vbo);

	for (const auto &constraint : api.getDistanceConstraints())
	{
		auto bodyA = api.getBody(constraint.firstBodyId);
		auto bodyB = api.getBody(constraint.secondBodyId);
		if (!bodyA || !bodyB)
			continue;
		glm::vec3 p0(bodyA->position.x, bodyA->position.y, bodyA->position.z);
		glm::vec3 p1(bodyB->position.x, bodyB->position.y, bodyB->position.z);
		std::vector<float> solid = buildSolidConstraint(constraint, p0, p1);
		if (solid.empty())
			continue;
		glBufferData(GL_ARRAY_BUFFER, solid.size() * sizeof(float), solid.data(), GL_DYNAMIC_DRAW);
		float r, g, b;
		constraintBaseColor(constraint.type, r, g, b);
		...
		if (smMatShininess >= 0)
			glUniform1f(smMatShininess, constraint.type == ConstraintType::Rope ? 24.0f : constraint.type == ConstraintType::Rod ? 96.0f : 48.0f);
		if (smCol >= 0)
    ...
    float tintB)
{
	...
	for (const auto &constraint : api.getDistanceConstraints())
	{
		...
		glm::vec3 p0(bodyA->position.x, bodyA->position.y, bodyA->position.z);
		glm::vec3 p1(bodyB->position.x, bodyB->position.y, bodyB->position.z);
		std::vector<float> lines = buildWireConstraint(constraint, p0, p1);
		if (lines.empty())
			continue;
