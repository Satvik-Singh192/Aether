...
{
	Rigidbody copy = body;
	if (copy.collider && copy.collider->type == ShapeType::Ramp)
	{
		auto *ramp = static_cast<RampCollider *>(copy.collider);
		const Vec3 comLocal = ramp->getLocalCenterOfMassOffset();
		const Mat3 rot = copy.orientation.toMat3();
		copy.position = copy.position + (rot * comLocal);
	}
...
{
	if (body.collider && body.collider->type == ShapeType::Ramp)
	{
		auto *ramp = static_cast<RampCollider *>(body.collider);
		const Vec3 comLocal = ramp->getLocalCenterOfMassOffset();
		const Mat3 rot = body.orientation.toMat3();
		body.position = body.position + (rot * comLocal);
	}
...
void PhysicsWorld::step(float dt)
{
	...
	for (auto &body : bodies)
	{
		...
		if (std::abs(body.velocity.x) < 0.01f)
			body.velocity.x = 0;
		if (std::abs(body.velocity.z) < 0.01f)
			body.velocity.z = 0;
